...
public class FileProjectStore : IProjectStore {
    ...
    public IEnumerable<string> ListProjects() {
        ...
        foreach (var dir in Directory.GetDirectories(_rootDir)) yield return Path.GetFileName(dir);
    }
    ...
    public IEnumerable<CodeFileMetadata> ListFiles(string projectName) {
        ...
        var items = JsonSerializer.Deserialize<List<CodeFileMetadata>>(File.ReadAllText(metaPath)) ??
                    new List<CodeFileMetadata>();
    ...
    public CodeFileMetadata AddFile(string projectName, string originalFilePath, bool overwriteIfNameExists,
        ...
        if (existing == null) items.Add(metadata);
        SaveMeta(projectName, items);
        return metadata;
    }
    ...
    private string GetProjectDir(string projectName) {
        return Path.Combine(_rootDir, projectName);
    }
